...
export async function GetCategory() {
    const response = await fetch('https://projet-apis.herokuapp.com/api/v1/category');
    const categories = await response.json();
    return categories;
}

export async function GetCategoryItem(id) {
    const response = await fetch(`https://projet-apis.herokuapp.com/api/v1/category/${id}/list`);
    const categories = await response.json();
    return categories;
}
